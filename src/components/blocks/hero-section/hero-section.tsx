        ...
        <div className='grid grid-cols-1 gap-6 gap-y-12 md:gap-y-16 lg:grid-cols-5'>
          <div className='flex w-full flex-col justify-center gap-5 max-lg:items-center lg:col-span-3 lg:h-95.5'>
            <h1 className='text-3xl leading-[1.29167] font-semibold text-balance max-lg:text-center sm:text-4xl lg:text-5xl'>
              Culture carried forward together
            </h1>

            <p className='text-muted-foreground max-w-xl text-xl max-lg:text-center'>
              Bamena-USA connects families, honors elders, mentors youth, and funds programs that keep our heritage
              visible, useful, and shared.
            </p>
              ...
              {menudata.map(item => (
                <CarouselItem key={item.id} className='flex w-full items-center justify-center'>
                  <img
                    src={item.img}
                    alt={item.imgAlt}
                    className='size-95 rounded-md object-cover object-[center_40%]'
                  />
                </CarouselItem>
              ))}
            </CarouselContent>
                ...
                >
                  <div className='relative flex h-33 items-center justify-center'>
                    ...
                    <img
                      src={item.img}
                      alt={item.imgAlt}
                      className='size-25 rounded-md object-cover object-[center_40%]'
                    />
                  </div>
                </CarouselItem>
              ))}
              ...
              {menudata.map(item => (
                ...
                >
                  <img
                    src={item.userAvatar}
                    alt={item.imgAlt}
                    className='size-10 rounded-full object-cover object-[center_38%]'
                  />
